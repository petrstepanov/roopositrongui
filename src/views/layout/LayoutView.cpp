...
LayoutView::LayoutView(const TGWindow *w) :
    AbstractView<LayoutPresenter>(w) {
  // Build UI
  initUI();
...
void LayoutView::initUI() {
  ...
  AddFrame(toolbar,
      new TGLayoutHints(kLHintsLeft | kLHintsTop | kLHintsExpandX, Padding::dx, Padding::dx, Padding::dx, Padding::dx));
...
//    Int_t parts[] = {100};
  ...
  statusBar->SetText("Welcome to RooLifetime. Press \"New Project\" to start or \"Open\" to load existing project.", 0);
}
...
void LayoutView::createNewProjectView() {
  ...
  projectContainer->AddFrame(newProjectView,
