...
TH1* HistUtils::cutHist(TH1* histogram, Int_t minBin, Int_t maxBin){
	...
	TH1* cutHistogram = new TH1I(name.Data(), title.Data(), maxBin - minBin + 1, lowMinBinEdge, upMaxBinEdge);
...
RooDataHist* HistUtils::makeRooDataHist(TH1* histogram, RooRealVar* channels){
	...
	RooDataHist* dataHist = new RooDataHist(name.Data(), title.Data(), RooArgList(*channels), histogram);
	return dataHist;
