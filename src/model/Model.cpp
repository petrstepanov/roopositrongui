...
Bool_t Model::saveProjectToFile(const TString *filename) {
  ...
  // create a ROOT Tree
  // TTree t4("t4", "A Tree with ProjectModel", 99, gDirectory);
  TTree t4("t4", "Tree with ProjectModel");
  // create a pointer to an Event object
  // create two branches, split one
  t4.Branch("projectmodel", "ProjectModel", projectModel, 16000, 2);
...
void Model::setTrimChannels(Int_t minChannel, Int_t maxChannel) {
  if ((minChannel >= 1 && minChannel <= projectModel->numberOfChannels) && //
    ...
    TVector2 *vector = new TVector2(projectModel->minTrimChannel,
        projectModel->maxTrimChannel);
